...
export function GmailMockFlow(){
  ...
  if(gmailStep===1)return(<Wrap><div style={{display:"flex",flexDirection:"column",paddingTop:32}}><GLogo size={36}/><div style={{fontSize:20,fontWeight:500,color:C.text,marginTop:18}}>CardCanvas wants access to your Google Account</div><div style={{display:"flex",alignItems:"center",gap:10,marginTop:14,padding:"10px 12px",background:C.bg,borderRadius:10}}><div style={{width:28,height:28,borderRadius:"50%",background:"#4285F4",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:600}}>A</div><div style={{fontSize:13,color:C.text}}>aarav.sharma@gmail.com</div></div><div style={{fontSize:13,color:C.sub,marginTop:24,marginBottom:10,fontWeight:600}}>This will allow CardCanvas to:</div>{["Read transaction emails from your banks","Identify card names and benefit details","Track your statements securely"].map((p,i)=>(<div key={i} style={{display:"flex",gap:12,padding:"10px 0",borderBottom:i<2?`1px solid ${C.brd}`:"none"}}><div style={{color:C.green,fontSize:16,marginTop:1}}>✓</div><div style={{fontSize:13,color:C.text,lineHeight:1.5}}>{p}</div></div>))}<div style={{fontSize:11,color:C.dim,marginTop:18,lineHeight:1.5}}>You can review CardCanvas's terms and privacy policies before deciding. You can manage permissions in your Google Account.</div><div style={{display:"flex",gap:10,marginTop:28}}><div onClick={()=>setScreen(gmailReturnTo||"home")} style={{flex:1,padding:"12px",borderRadius:8,textAlign:"center",fontSize:14,fontWeight:600,color:C.blue,cursor:"pointer"}}>Cancel</div><div onClick={()=>{setGmailStep(0);setScreen("gmail-extra");}} style={{flex:1,padding:"12px",borderRadius:8,background:C.blue,color:"#fff",textAlign:"center",fontSize:14,fontWeight:600,cursor:"pointer"}}>Allow</div></div></div></Wrap>);
  if(gmailStep===2){
